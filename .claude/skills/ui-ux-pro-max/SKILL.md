...
`$SKILL` below stands for this skill's own directory. **Shell state does not persist between tool calls, so substitute the literal path into every command** rather than exporting a variable:
...
```bash
python "$SKILL/scripts/search.py" "<query>" --domain <domain>
```

If `python` is not found, try `python3`, then `py -3`. Requires Python 3.x, no external dependencies.

## In FireGuard, this skill advises — it does not overrule the repo

The database is stack-agnostic and will happily suggest a component library, a font, or an animation library this repo does not use. FireGuard Web's own rules win every time:

- **Stack is always `--stack angular`.** Never `shadcn`, `react`, or `html-tailwind`, even when a query's phrasing leans that way.
- **spartan/ui is the component library** (see the `spartan-ui` skill). Compose existing helm components; never hand-roll a control the catalog already ships, and never take a "build a custom X" recommendation literally.
- **Colors and typography come from the theme tokens** in `src/styles.css`, with `html[data-theme="dark"]` parity. Treat the palette and font-pairing output as _evaluation criteria_ (contrast, hierarchy, role separation) for the existing tokens — not as a new palette to paste in. Do not introduce a Google Font or a raw hex.
- **No new dependencies.** GSAP presets are structural advice about timing, easing, and staging; express them with what the app already has. Adding a package needs the user's go-ahead.
- **Do not run `--persist`.** `PRODUCT.md` and `ARCHITECTURE.md` are the design source of truth; a generated `design-system/` tree would be a second, competing one.
- The accessibility and interaction rules (priorities 1, 2, 5, 8) _are_ directly applicable — they line up with `PRODUCT.md` and the `fg-a11y` audit, and are the highest-value part of this database here.

Where this skill's advice and the repo's rules disagree, follow the repo and say so instead of silently splitting the difference.
...
```bash
python "$SKILL/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
```bash
python "$SKILL/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```

### Step 2b: Persist Design System (Master + Overrides Pattern)

> **Not used in FireGuard** — see the constraints section above. `PRODUCT.md` and `ARCHITECTURE.md` already hold the design source of truth; keep this step for reference only.
...
```bash
python "$SKILL/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "$SKILL/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
```bash
python "$SKILL/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "$SKILL/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "$SKILL/scripts/search.py" "<keyword>" --stack <stack>
```

**Available stacks:** `react`, `nextjs`, `vue`, `svelte`, `astro`, `nuxtjs`, `nuxt-ui`, `angular`, `laravel`, `swiftui`, `react-native`, `flutter`, `jetpack-compose`, `html-tailwind`, `shadcn`, `threejs`, `javafx`, `wpf`, `winui`, `avalonia`, `uno`, `uwp`. Use the stack detected in Step 1 — in FireGuard Web that is always `angular`.
...
python "$SKILL/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "$SKILL/scripts/search.py" "search loading animation" --domain ux

# Step 4: stack guidelines
python "$SKILL/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
...
Vendored from [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) (MIT, `LICENSE` kept alongside), commit `abb7f2f`, 2026-08-11. Local changes: `$SKILL` path resolution for the plugin/app-root split, the FireGuard constraints section, the Step 2b and Step 4 notes, and this footer. Re-vendoring upstream means reapplying them.
