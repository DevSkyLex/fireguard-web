...
describe('InterventionsPage', () => {
  ...
  describe('list navigation and filter controls', () => {
    it.each([
      { key: 'status', cleared: { status: null } },
      { key: 'type', cleared: { type: null } },
      { key: 'priority', cleared: { priority: null } },
      { key: 'site', cleared: { site: null } },
      { key: 'responsible', cleared: { responsible: null } },
      { key: 'label', cleared: { label: null } },
      { key: 'dueRange', cleared: { dueAfter: null, dueBefore: null } },
      { key: 'plannedStartRange', cleared: { plannedStartAfter: null, plannedStartBefore: null } },
    ])(
      'removes only the $key narrowing and preserves the other URL filters',
      async ({ key, cleared }) => {
        fixture = await createPage({
          q: 'north',
          status: 'planned',
        ...
        fixture.componentInstance['onFieldRemoved'](key);
        expect(navigate).toHaveBeenLastCalledWith(
          [],
          expect.objectContaining({
            queryParams: expect.objectContaining({ ...cleared, p: null }),
            queryParamsHandling: 'merge',
          }),
        );
        const lastParams = navigate.mock.lastCall?.[1].queryParams;
        expect(lastParams).not.toHaveProperty('q');
        expect(key === 'status' ? lastParams.type : lastParams.status).toBe(
          key === 'status' ? 'inventory' : 'planned',
        );
      },
    );

    it('clears a former planned-start range when its operator changes, preserving its open editor', async () => {
      fixture = await createPage({ plannedStartAfter: '2026-08-01' });
      const page = fixture.componentInstance;
      page['onFilterOperatorChanged']({ key: 'plannedStartRange', operator: 'between' });
      expect(page['plannedStartRangeOperator']()).toBe('between');
      expect(page['fieldPopoverState']('plannedStartRange')).toBe('open');
      expect(navigate).toHaveBeenLastCalledWith(
        [],
        expect.objectContaining({
    ...
    it('clears selected rows when leaving selection mode and clears all filters through the URL', async () => {
      interventionList.set([intervention({ id: 'selected' })]);
      fixture = await createPage({ status: 'planned', priority: 'high' });
      const page = fixture.componentInstance;
      page['toggleSelectionMode']();
      page['onSelectionChanged'](new Set(['selected']));
      expect(page['selectionMode']()).toBe(true);
      page['toggleSelectionMode']();
      expect(page['selectionMode']()).toBe(false);
    ...
    it('represents scalar and repeated filters without inventing a scalar for multiple values', async () => {
      ...
      page['applyStatusFilter'](['draft', 'planned']);
      expect(navigate).toHaveBeenLastCalledWith(
        [],
        expect.objectContaining({
          queryParams: expect.objectContaining({ status: 'draft,planned' }),
        }),
      );
      page['applyTypeFilter'](['inventory']);
      expect(navigate).toHaveBeenLastCalledWith(
        [],
        expect.objectContaining({ queryParams: expect.objectContaining({ type: 'inventory' }) }),
      );
