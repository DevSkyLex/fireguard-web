...
export class Calendar {
   ...
   */
  protected readonly activeView: WritableSignal<CalendarView> = signal<CalendarView>('month');
   ...
   */
  protected readonly activeDate: WritableSignal<Date> = signal<Date>(startOfDay(new Date()));
   ...
   * @type {0 | 1}
   */
  protected readonly weekStartsOn: 0 | 1 = CALENDAR_DEFAULT_WEEK_STARTS_ON;
   ...
   *
   * @description
   * Month-cell chip cap before the "+N more" affordance.
   *
   * @access protected
   * @since 1.0.0
   *
   * @type {number}
   */
  protected readonly maxEventsPerDay: number = CALENDAR_DEFAULT_MAX_EVENTS_PER_DAY;
   ...
   * @type {number}
   */
  protected readonly dayStartHour: number = CALENDAR_DEFAULT_DAY_START_HOUR;
   ...
   * @type {number}
   */
  protected readonly dayEndHour: number = CALENDAR_DEFAULT_DAY_END_HOUR;
   ...
   *
   * @description
   * Views offered by the toolbar switch.
   *
   * @access protected
   * @since 1.0.0
   *
   * @type {readonly ViewOption[]}
   */
  protected readonly viewOptions: readonly ViewOption[] = (
    ['month', 'week', 'agenda'] as const
  ).map((value: CalendarView): ViewOption => ({ label: VIEW_LABELS[value], value }));
