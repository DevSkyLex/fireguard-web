...
const query = <T extends HTMLElement>(
  fixture: ComponentFixture<AccountMenu>,
  testid: string,
...
describe('AccountMenu', () => {
  const setup = (
    ...
    const fixture: ComponentFixture<AccountMenu> = TestBed.createComponent(AccountMenu);
  ...
  describe('loading', () => {
    it('should render a skeleton while the identity is loading', () => {
      ...
      expect(query(fixture, 'account-menu-trigger')).toBeNull();
    });
  });
  ...
  describe('loaded — trigger', () => {
    it('should render the trigger button once loaded', () => {
      ...
      expect(query(fixture, 'account-menu-trigger')).toBeTruthy();
    ...
    it('should reflect the popover open state on aria-expanded', () => {
      ...
      expect(query(fixture, 'account-menu-trigger')?.getAttribute('aria-expanded')).toBe('false');

      query<HTMLButtonElement>(fixture, 'account-menu-trigger')?.click();
      fixture.detectChanges();

      expect(query(fixture, 'account-menu-trigger')?.getAttribute('aria-expanded')).toBe('true');
    });
  });
  ...
  describe('popover destinations', () => {
    it('should link the account destinations to /account', () => {
      ...
      const profile = queryInPopover<HTMLAnchorElement>('account-menu-profile');
      const notifications = queryInPopover<HTMLAnchorElement>('account-menu-notifications');
    ...
    it('should close the popover after activating a destination link', () => {
      ...
      queryInPopover<HTMLAnchorElement>('account-menu-profile')?.click();
      fixture.detectChanges();
  ...
  describe('logout', () => {
    it('should trigger logout from the popover action', () => {
      ...
      queryInPopover<HTMLButtonElement>('account-menu-logout')?.click();
    ...
    it('should render "Logging out..." and disable the logout button while logging out', () => {
      ...
      const logoutButton = queryInPopover<HTMLButtonElement>('account-menu-logout');
