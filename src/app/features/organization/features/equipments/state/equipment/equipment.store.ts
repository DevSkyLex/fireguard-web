  ...
  withMethods((
    ...
    return {
      ...
      assignToFacility: rxMethod<{ organizationId: string; equipmentId: string; input: AssignToFacilityInput }>(
        pipe(
          tap((): void => { patchState(store, { assignToFacilityCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId, input }) =>
                ...
                error: (error: unknown): void => {
                  ...
                  dispatcher.dispatch(
                    equipmentStoreEvents.assignToFacilityFailed(
                      toStoreFailureEventPayload(storeError, 'Failed to assign equipment to facility'),
                    ),
                  );
                },
              }),
      ...
      unassignFromFacility: rxMethod<{ organizationId: string; equipmentId: string }>(
        pipe(
          tap((): void => { patchState(store, { unassignFromFacilityCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId }) =>
            equipmentService.unassignFromFacility(organizationId, equipmentId).pipe(
              tapResponse({
                ...
                error: (error: unknown): void => {
                  const storeError: StoreError = toStoreError(error);
                  patchState(store, { unassignFromFacilityCallState: errorCallState(storeError) });
                  dispatcher.dispatch(
                    equipmentStoreEvents.unassignFromFacilityFailed(
                      toStoreFailureEventPayload(storeError, 'Failed to unassign equipment from facility'),
                    ),
                  );
                },
              }),
      ...
      commission: rxMethod<{ organizationId: string; equipmentId: string }>(
        pipe(
          tap((): void => { patchState(store, { commissionCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId }) =>
      ...
      decommission: rxMethod<{ organizationId: string; equipmentId: string }>(
        pipe(
          tap((): void => { patchState(store, { decommissionCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId }) =>
      ...
      maintenance: rxMethod<{ organizationId: string; equipmentId: string }>(
        pipe(
          tap((): void => { patchState(store, { maintenanceCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId }) =>
                ...
                error: (error: unknown): void => {
                  ...
                  dispatcher.dispatch(
                    equipmentStoreEvents.maintenanceFailed(
                      toStoreFailureEventPayload(storeError, 'Failed to set equipment to maintenance'),
                    ),
                  );
                },
              }),
      ...
      loadAttachments: rxMethod<{ organizationId: string; equipmentId: string; options?: RequestOptions }>(
        pipe(
          tap((): void => { patchState(store, { attachmentsListCallState: pendingCallState() }); }),
      ...
      addAttachment: rxMethod<{ organizationId: string; equipmentId: string; input: AddAttachmentInput }>(
        pipe(
          tap((): void => { patchState(store, { addAttachmentCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId, input }) =>
      ...
      deleteAttachment: rxMethod<{ organizationId: string; equipmentId: string; attachmentId: string }>(
        pipe(
          tap((): void => { patchState(store, { deleteAttachmentCallState: pendingCallState() }); }),
      ...
      loadTags: rxMethod<{ organizationId: string; search?: string; options?: RequestOptions }>(
        pipe(
          tap((): void => { patchState(store, { tagsListCallState: pendingCallState() }); }),
      ...
      addTag: rxMethod<{ organizationId: string; equipmentId: string; input: AddTagInput }>(
        pipe(
          tap((): void => { patchState(store, { addTagCallState: pendingCallState() }); }),
          exhaustMap(({ organizationId, equipmentId, input }) =>
      ...
      removeTag: rxMethod<{ organizationId: string; equipmentId: string; tagId: string }>(
        pipe(
          tap((): void => { patchState(store, { removeTagCallState: pendingCallState() }); }),
