...
import { DirectConversationsStore } from '@features/collaboration/state';
...
export class CollaborationDirectNav {
   ...
   *
   * @description
   * Workspace-scoped prefix every conversation destination is built from.
   *
   * @access private
   * @since 1.0.0
   *
  ...
  private readonly routePrefix: Signal<string | null> = computed((): string | null => {
    ...
    return organization ? `/organizations/${organization.id}/workspace/direct` : null;
  });
  ...
  public constructor() {
    effect((): void => {
      ...
      // load transition.
      untracked((): void => {
        this.store.ensureLoaded(organization.id);
  ...
  protected startWith(memberId: string): void {
    ...
    this.store.open({ organization: organization.id, memberId });
  }
  //#endregion
}
