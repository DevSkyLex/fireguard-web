  ...
  withComputed((store) => {
    ...
    return {
      ...
      isEmpty: computed<boolean>(
        () => store.checklistIds().length === 0 && !store.isLoading(),
      ),
      ...
      isArchiving: computed<boolean>(
        () => store.archiveOperation().status === 'loading',
      ),
      ...
      createError: computed<OperationError<unknown> | null>(() => {
         ...
         */
        const operation: Operation<ChecklistOutput | null, unknown> = store.createOperation();
  ...
  withMethods((
            ...
            tapResponse({
              next: (response: HydraCollection<ChecklistOutput>): void => {
                patchState(
                  ...
                  { totalChecklists: response.totalItems, isLoading: false },
                );
              },
    ...
    return {
      ...
      resetArchiveOperation(): void {
        patchState(store, { archiveOperation: createIdleOperation() });
      },
    };
  }),
  //#endregion
