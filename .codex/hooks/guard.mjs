 ...
 *     takes theme tokens, at-rules, and element resets only (DESIGN.md).
 ...
 *     HTML comment — prose belongs in the doc block (.codex/rules/comments.md).
 *
  ...
  if (componentRule.test(body)) {
    deny(
      'src/styles.css takes the spartan/ui theme (layers, the brain preset import, and ' +
        ...
        'Tailwind utility classes at the call site (DESIGN.md, ARCHITECTURE.md §8.5).',
    );
  }
}
...
if (
  isBarrel &&
  !/(^|\/)src\/app\/shared\/ui\//.test(filePath) &&
  /^\s*export\s+\*\s+from\s/m.test(written)
) {
  deny(
...
// line is left to review (.codex/rules/comments.md, ARCHITECTURE.md §14.4).
  ...
  if (/^[ \t]*\/\/(?!#).*\r?\n[ \t]*\/\/(?!#)/m.test(written)) {
    deny(
      `${filePath} has a multi-line "//" comment block. Two or more comment lines in a ` +
        ...
        'mistake is the only inline form allowed (.codex/rules/comments.md).',
    );
  }
}
  ...
  if (/<!--(?!\s*(prettier|oxfmt)-ignore)/.test(written)) {
    deny(
      `${filePath} has an HTML comment. Templates carry no rationale — a layout, an ` +
        ...
        '@description, not beside the element (.codex/rules/comments.md).',
    );
  }
}
