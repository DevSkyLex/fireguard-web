...
RUN APP_API_URL="$APP_API_URL" \
  ...
  node -e 'const fs = require("node:fs"); const maintenanceValue = (process.env.APP_MAINTENANCE ?? "false").toLowerCase(); const maintenance = maintenanceValue === "true"; const fileContent = `import { type EnvironmentConfig } from "@core/config/environment";\n\n/**\n * Environment production\n * @type {EnvironmentConfig}\n *\n * @description\n * This file is generated during the Docker image build.\n */\nexport const environment: EnvironmentConfig = {\n  production: true,\n  apiUrl: ${JSON.stringify(process.env.APP_API_URL ?? "http://localhost:8000")},\n  appName: ${JSON.stringify(process.env.APP_NAME ?? "Fireguard")},\n  mercureHubUrl: ${JSON.stringify(process.env.APP_MERCURE_HUB_URL ?? "http://localhost:3000/.well-known/mercure")},\n  maintenance: ${maintenance},\n};\n`; fs.writeFileSync("src/environments/environment.ts", fileContent, "utf8");'
