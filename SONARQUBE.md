...
The main-branch integration also brings the full-source coverage configuration
and report artifact already used on develop. It does not promote develop's
application or test backlog. Main must independently meet the 90% line threshold;
if its first full coverage run fails, keep readiness false and address that
baseline through the normal release process before activation.
