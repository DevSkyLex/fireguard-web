...
import { DirectMessagesPanel } from '../direct-messages-panel.component';
...
describe('DirectMessagesPanel', () => {
  let fixture: ComponentFixture<DirectMessagesPanel>;
  ...
  function group(): HTMLElement | null {
    return fixture.nativeElement.querySelector('#direct-messages-panel');
  }
  ...
  beforeEach(async () => {
    ...
    fixture = TestBed.createComponent(DirectMessagesPanel);
  ...
  it('should offer to start a conversation only with the write permission', async () => {
    expect(
      fixture.nativeElement.querySelector('[data-testid="direct-messages-panel-new"]'),
    ).not.toBeNull();
  ...
  it('filters resolved names without case or accent sensitivity and recovers from no matches', async () => {
    const input: HTMLInputElement = fixture.nativeElement.querySelector('input[type="search"]');
    input.value = ' AMELIE ';
    input.dispatchEvent(new Event('input'));
    await fixture.whenStable();
    expect(
      fixture.nativeElement.querySelectorAll('[data-testid="direct-messages-panel-row"]'),
    ).toHaveLength(1);
    input.value = 'Nobody';
    input.dispatchEvent(new Event('input'));
    await fixture.whenStable();
    expect(text()).toContain('No matching conversations.');
    input.value = '';
    input.dispatchEvent(new Event('input'));
    await fixture.whenStable();
    expect(text()).toContain('Amélie Rousseau');
