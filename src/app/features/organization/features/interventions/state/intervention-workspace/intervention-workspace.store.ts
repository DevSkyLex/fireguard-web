    ...
    (
          ...
          concatMap((workspace) =>
            from(offline.listOutbox(interventionId)).pipe(
              switchMap((operations) => {
                if (!operations.length) return of(workspace);
                return from(offline.getWorkspace(interventionId)).pipe(
                  map((saved) => {
                    const projected = projectInterventionWorkspace(
                      { ...workspace, issues: workspace.issues.member },
                      operations,
                      saved ?? undefined,
                    );
                    return { ...workspace, ...projected, issues: { member: projected.issues } };
                  }),
                );
              }),
            ),
      ...
      const applyWorkspace = ({
        ...
        patchState(store, {
          ...
          issuesCallState: fromCache ? idleCallState() : successCallState(null),
      ...
      const requestWorkspace = rxMethod<{
        ...
        pipe(
          switchMap(({ interventionId, generation, reload: reloading }) => {
            const current = (): boolean =>
              store.contextId() === interventionId && store.loadGeneration() === generation;
            return fetchWorkspace(interventionId).pipe(
              tapResponse({
                next: (workspace) => {
                  if (!current() || workspace.intervention.id !== interventionId) return;
                  applyWorkspace(workspace);
                  if (reloading)
                    dispatcher.dispatch(
                      interventionWorkspaceStoreEvents.reloadSucceeded({ interventionId }),
                    );
                },
                error: (error: unknown) => {
                  if (current())
      ...
      function load(interventionId: string): void {
        ...
        requestWorkspace({ interventionId, generation, reload: false });
      }
      ...
      function reload(interventionId: string): void {
        ...
        const generation = store.loadGeneration() + 1;
        patchState(store, { loadGeneration: generation, loadCallState: pendingCallState() });
        requestWorkspace({ interventionId, generation, reload: true });
      }
      ...
      const refreshIssues = rxMethod<string>(
        pipe(
          tap(() => patchState(store, { issuesCallState: pendingCallState() })),
          switchMap((interventionId) =>
            service.listIssues(interventionId).pipe(
              tapResponse({
                next: (collection) => {
                  if (store.intervention()?.id === interventionId)
                    patchState(store, {
                      issues: collection.member,
                      issuesCallState: successCallState(null),
                    });
                },
                error: (error: unknown) => {
                  if (store.intervention()?.id === interventionId)
                    patchState(store, { issuesCallState: errorCallState(toStoreError(error)) });
                },
              }),
            ),
          ),
            ...
            service.listActivities(interventionId).pipe(
              ...
              tapResponse({
                next: ({ page, collection }) =>
                  (store.contextId() === null || store.contextId() === interventionId) &&
                  patchState(store, {
                ...
                error: (error: unknown) => {
                  if (store.contextId() !== null && store.contextId() !== interventionId) return;
      ...
      return {
        ...
        createWorkItem: rxMethod<InterventionWorkItemCreateCommand>(
          pipe(
            exhaustMap(({ interventionId, input }) => {
              const clientId = input.clientId ?? crypto.randomUUID();
              const payload = { ...input, clientId };
              const captured = store.intervention();
              const current = (): boolean =>
                store.intervention()?.id === captured?.id &&
                store.intervention()?.organization === captured?.organization;
              patchState(store, { createWorkItemCallState: pendingCallState() });
              let queued = false;
              const queue = () =>
                defer(() => {
                  queued = true;
                  return offline.queue(interventionId, 'work-item.create', payload);
                }).pipe(map(() => optimistic.createWorkItem(payload, clientId)));
              ...
              return request.pipe(
                tapResponse({
                  next: (created: InterventionWorkItemOutput) => {
                    if (!current()) return;
                    const workItems = [...store.workItems(), created];
                    const intervention = optimistic.addWorkItem(store.intervention());
                    patchState(store, {
                      workItems,
                      intervention,
                    ...
                    dispatcher.dispatch(
                      interventionWorkspaceStoreEvents.workItemCreateSucceeded({ interventionId }),
                    );
                    dispatcher.dispatch(
                      interventionWorkspaceStoreEvents.mutationSucceeded({
                        interventionId,
        ...
        setWorkItemStatus: rxMethod<InterventionWorkItemStatusCommand>(
          pipe(
            mergeMap(({ interventionId, workItemId, status, skipReason }) => {
              if (store.pendingWorkItemIds().has(workItemId)) return EMPTY;
              const captured = store.intervention();
              const current = (): boolean =>
                store.intervention()?.id === captured?.id &&
                store.intervention()?.organization === captured?.organization;
              const item = store.workItems().find((candidate) => candidate.id === workItemId);
              const reason = status === 'skipped' ? (skipReason ?? item?.skipReason ?? null) : null;
              patchState(store, {
                workItemWriteCallState: pendingCallState(),
                pendingWorkItemIds: withId(store.pendingWorkItemIds(), workItemId),
                workItemErrors: { ...store.workItemErrors(), [workItemId]: null },
              });
              let queued = false;
              const queue = () =>
                defer(() => {
              ...
              const request =
                connectivity.isOffline() && item
                  ? queue()
                  : service
                      .updateWorkItem(workItemId, { status, skipReason: reason }, item?.revision)
                      .pipe(
                        catchError((error: unknown) =>
                          item && connectivity.isNetworkFailure(error)
                            ? queue()
                            : throwError(() => error),
                        ),
                      );
                  ...
                  next: (updated) => {
                    ...
                    dispatcher.dispatch(
                      interventionWorkspaceStoreEvents.mutationSucceeded({
                        ...
                        workItem: updated ?? result.workItem,
                      }),
                    );
          ...
          pipe(
            ...
            mergeMap(({ interventionId, changeId }) => {
              ...
              patchState(store, {
                ...
                changeErrors: { ...store.changeErrors(), [changeId]: null },
              });
              ...
              const applyRejection = (
                rejected: InterventionChangeOutput,
                source: 'remote' | 'queued' = 'remote',
              ): void => {
                if (!current()) return;
                const changes: readonly InterventionChangeOutput[] = store
                  .changes()
                  .map((row) => (row.id === changeId ? rejected : row));
                patchState(store, {
                  changes,
              ...
              return service.updateChange(changeId, { status: 'rejected' }, change.revision).pipe(
                ...
                catchError((error: unknown) => {
                  ...
                  patchState(store, {
                    rejectChangeCallState: errorCallState(toStoreError(error)),
                    changeErrors: {
                      ...store.changeErrors(),
                      [changeId]: toStoreError(error).message,
                    },
                  });
                  dispatcher.dispatch(
          ...
          pipe(
            ...
            switchMap(({ interventionId, workItems }) => {
              ...
              return forkJoin(
                ...
                tapResponse({
                  next: () => {
                    if (store.intervention()?.id !== interventionId) return;
                  ...
                  error: (error: unknown) =>
                    store.contextId() === interventionId &&
                    patchState(store, {
                      deleteWorkItemsCallState: errorCallState(
                        workspaceFailure(
                          error,
        ...
        loadAttachments: rxMethod<string>(
          pipe(
            ...
            switchMap((interventionId) =>
              forkJoin({
                collection: service.listAttachments(interventionId).pipe(
                  map((collection) => ({ collection, error: null })),
                  catchError((error: unknown) =>
                    of({ collection: null, error: toStoreError(error) }),
                  ),
                ),
                ...
                tapResponse({
                  next: ({ collection, queued }) =>
                    patchState(store, {
                      attachments: collection.collection?.member ?? store.attachments(),
                      queuedAttachments: queued,
                      attachmentsCallState: collection.error
                        ? errorCallState(collection.error)
