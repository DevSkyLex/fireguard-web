 ...
 * matching the organization table badges and dashboard trend-card filter selects.
 * Dark variants keep the colour legible on the dark surface. Each value is a
 * complete literal string so Tailwind's content scanner keeps the utilities.
 */
const SEVERITY_ICON_CLASS: Record<InterventionTagSeverity, string> = {
 ...
 *
 * @param severity - Severity colour role.
 * @returns The matching `text-*` utility class string.
 */
export function interventionSeverityIconClass(severity: InterventionTagSeverity): string {
