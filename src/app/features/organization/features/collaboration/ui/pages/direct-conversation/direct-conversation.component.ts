...
@Component({
  ...
  providers: [MessageRepliesStore, MessageThreadStore],
  templateUrl: './direct-conversation.component.html',
  host: { class: 'flex min-h-0 flex-1 flex-col', 'data-testid': 'direct-conversation' },
...
export class DirectConversationPage {
   ...
   */
  protected readonly messageStatus: Signal<ReadonlyMap<string, 'pending' | 'failed'>> = computed(
  ...
  public constructor() {
    ...
    effect((): void => {
      ...
      // A thread left open over the previous conversation would show its
      // replies above the new one.
      this.replies.close();
    });
   ...
   *
   * @description
   * Moves the read marker to a message, which is what clears the
   * conversation's unread badge from a point the member chooses rather than
   * from now.
   *
   * @access protected
   * @since 1.3.0
