---
description: Create an Angular component — presentational, page, p-table grid, p-dataview surface, form, dialog, or sheet — as a complete unit folder following ARCHITECTURE.md §10.2.
...
1. **Decide placement first** and state the rule that drove it — `ui/pages/` (class ends `Page`, no barrel), `ui/tables/`, `ui/dataviews/`, `ui/forms/`, `ui/dialogs/`, `ui/sheets/`, `ui/components/`, or `shared/<concept>/ui/components/`. Before anything lands in `shared/`, apply the §8.5 test: a wrapper that only avoids repeating markup **does not earn its place**.
