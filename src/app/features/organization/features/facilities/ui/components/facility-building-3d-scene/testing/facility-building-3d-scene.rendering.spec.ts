...
/**
 * @description Keeps real geometry, materials and picking while substituting jsdom's unavailable GPU boundary.
 */
...
describe('FacilityBuilding3dScene', () => {
  ...
  it('rebuilds the current model on a theme change while retaining selection and isolation', async () => {
    await mount();
    const scene = fixture.componentInstance;
    fixture.componentRef.setInput('selectedRoomId', 'room-0');
    fixture.componentRef.setInput('isolatedFloorId', 'floor-0');
    await fixture.whenStable();
    const previous = sceneObject(scene['roomMeshes'].get('room-0'));
    const disposed = vi.spyOn(previous.geometry, 'dispose');
    resolvedTheme.set('dark');
    await fixture.whenStable();
    const replacement = sceneObject(scene['roomMeshes'].get('room-0'));
    expect(replacement).not.toBe(previous);
    expect(disposed).toHaveBeenCalledTimes(1);
    expect(scene['selectedRoomOutline']?.parent).toBe(replacement.parent);
  ...
  it('rebuilds geometry on model changes and frees shared resources once on context loss', async () => {
    ...
    await fixture.whenStable();
    canvas.dispatchEvent(new MouseEvent('pointermove', { clientX: 10 }));
    const pendingSceneFrames = [scene['renderRaf'], scene['hoverRaf'], scene['explodeRaf']];
    ...
    for (const id of pendingSceneFrames) {
      expect(id).not.toBeNull();
      if (id !== null) expect(frames.has(id)).toBe(false);
    }
