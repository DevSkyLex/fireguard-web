...
| Container — focused                                       | `mx-auto flex w-full max-w-md flex-col px-6 py-16 text-center sm:py-24` — upper-anchored with generous breathing. A `min-h-full` centring attempt does not resolve through the focused layout's auto-height ancestors; true vertical centring is a shell change (scheduled with the layout batch). |
