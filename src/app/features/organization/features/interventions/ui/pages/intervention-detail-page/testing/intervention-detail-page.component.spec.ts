...
describe('InterventionDetailPage', () => {
  ...
  let assignTeamCallState: WritableSignal<CallState>;
  ...
  beforeEach(() => {
    ...
    assignTeamCallState = signal<CallState>(idleCallState());
  ...
  describe('QR capture feedback', () => {
    it('reveals a matched work item without submitting a status change', async () => {
      const matched = workItem({ id: 'matched-work' });
      ...
      expect(fixture.componentInstance['activeLinkedTab']()).toBe('overview');
      expect(TestBed.inject(FeedbackService).success).toHaveBeenCalledWith('Found: matched-work');
      expect(setWorkItemStatus).not.toHaveBeenCalled();
    });
    it.each([
  ...
  describe('team assignment', () => {
    it('loads teams on demand, caches the result and closes assignment only after success', async () => {
      ...
      expect(store.assignTeam).toHaveBeenCalledExactlyOnceWith({
        interventionId: 'intervention-1',
        input: { teamId: 'team-1' },
      });
      assignTeamCallState.set(pendingCallState());
      await fixture.whenStable();
      expect(page['teamAssignVisible']()).toBe(true);
      assignTeamCallState.set(successCallState(null));
      await fixture.whenStable();
      expect(page['teamAssignVisible']()).toBe(false);
    });

    it('keeps failed team discovery retryable', async () => {
      const service = TestBed.inject(TeamService);
  ...
  it('routes label catalogue edits through the label store with the active organization', async () => {
    fixture = await createPage();
    const page = fixture.componentInstance;
    const labels = page['labelStore'];
    const reference = { destroy: vi.fn() };
    const loadLabels = vi.spyOn(labels, 'load').mockReturnValue(reference);
    const createLabel = vi.spyOn(labels, 'create').mockReturnValue(reference);
    const updateLabel = vi.spyOn(labels, 'update').mockReturnValue(reference);
    const removeLabel = vi.spyOn(labels, 'remove').mockReturnValue(reference);
    page['openManageLabels']();
    expect(loadLabels).toHaveBeenCalledExactlyOnceWith('/api/organizations/org-1');
    expect(page['manageLabelsVisible']()).toBe(true);
    page['createLabel']({ name: 'Priority', color: '#ff0000' });
    expect(createLabel).toHaveBeenCalledExactlyOnceWith({
      organization: '/api/organizations/org-1',
      name: 'Priority',
      color: '#ff0000',
    });
    page['updateLabel']({ labelId: 'label-1', name: 'Routine', color: '#00ff00' });
    expect(updateLabel).toHaveBeenCalledExactlyOnceWith({
      labelId: 'label-1',
      input: { name: 'Routine', color: '#00ff00' },
    });
    page['removeLabel']('label-1');
    expect(removeLabel).toHaveBeenCalledExactlyOnceWith('label-1');
    page['closeManageLabels']();
    expect(page['manageLabelsVisible']()).toBe(false);
  });
});
