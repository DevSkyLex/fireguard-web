...
import { getDashboardTrendPointValue, sumDashboardTrendValues } from '../organization-dashboard-trend.utils';
 ...
 *
 * @description
 * Represents a single entry in the equipment type select dropdown.
 *
 * @since 1.0.0
 */
type EquipmentTypeOption = {
 ...
 *
 * @description
 * Represents a single entry in the equipment status select dropdown,
...
/**
 * Type OrganizationDashboardEquipmentCreatedSummaryMetric
 *
 * @description
...
/**
 * Type OrganizationDashboardEquipmentCreatedState
 *
 * @description
...
type OrganizationDashboardEquipmentCreatedState = {
  ...
  readonly data: OrganizationDashboardTrendOutput | null;
 ...
 *
 * @description
 * Exhaustive list of selectable equipment types used to filter the trend query.
 *
 * @since 1.0.0
 *
 * @type {EquipmentTypeOption[]}
 ...
 *
 * @description
 * Exhaustive list of selectable equipment statuses including display
 * icon and color token used by the status select and legend.
 *
 * @since 1.0.0
 *
 * @type {EquipmentStatusOption[]}
...
export const OrganizationDashboardEquipmentCreatedStore = signalStore(
   ...
   * @since 1.0.0
   */
  withState<OrganizationDashboardEquipmentCreatedState>(INITIAL_STATE),
  //#endregion
  ...
  withComputed((store) => ({
     ...
     *
     * @description
     * Static list of equipment-type selector options. Exposed as a computed
     ...
     *
     * @description
     * Static list of equipment-status selector options. Exposed as a computed
     * signal for uniform binding.
     *
     * @since 1.0.0
     */
     ...
     *
     * @description
     * Full option object matching the currently selected equipment status.
    ...
    selectedEquipmentStatusOption: computed<EquipmentStatusOption | null>(
      () =>
        EQUIPMENT_STATUS_OPTIONS.find((o) => o.value === store.selectedEquipmentStatus()) ?? null,
    ),
     ...
     * Includes equipment-creation count with optional period-over-period comparison.
     *
     * @since 1.0.0
     */
    summaryMetrics: computed<readonly OrganizationDashboardEquipmentCreatedSummaryMetric[]>(
      () => {
        const trend = store.data();
