import type { InterventionChangePatchLine } from '@features/organization/features/interventions/models';
...
/**
 * Function interventionChangeResourceKind
 * @description Returns the same localized resource caption for table rendering and offline search.
 * @param {string} resource - Resource IRI supplied by the API.
 * @returns {string} Localized resource kind.
 * @since 6.2.0
 */
export function interventionChangeResourceKind(resource: string): string {
  if (resource.includes('/equipment/'))
    return $localize`:@@intervention.changes.resourceEquipment:Equipment`;
  if (resource.includes('/facilities/'))
    return $localize`:@@intervention.changes.resourceFacility:Facility`;
  if (resource.includes('/inspections/'))
