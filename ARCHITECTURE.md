...
A concept with **no UI** creates no `ui/` and stays flat (a validator, a util, a type, or a constants file plus their barrel — like `core/boot-readiness`). Illustrative concepts in this codebase: `layout-slot` (the slot mechanism the layouts are built on — section 8.2), `empty-state`, `error-state`, `identity-summary`, `inplace-field` (the edit-in-place shell of section 10.5), `password-input`, `theme-switcher`.
