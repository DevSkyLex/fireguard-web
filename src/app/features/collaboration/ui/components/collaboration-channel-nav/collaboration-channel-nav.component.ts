...
import { MemberPresenceService } from '@features/collaboration/services';
import { ChannelsStore } from '@features/collaboration/state';
import {
...
export class CollaborationChannelNav {
   ...
   *
   * @description
   * Workspace-scoped prefix every channel destination is built from, so a
  ...
  private readonly routePrefix: Signal<string | null> = computed((): string | null => {
    ...
    return organization ? `/organizations/${organization.id}/workspace/channels` : null;
  });
  ...
  protected readonly savedLink: Signal<readonly string[] | null> = computed(
    (): readonly string[] | null => {
      ...
      return organization ? [`/organizations/${organization.id}/workspace/saved`] : null;
    },
  );
  ...
  public constructor() {
    effect((): void => {
      ...
      this.store.load({ organization: organization.id });
      ...
      this.presence.start(organization.id);
    });
  }
  //#endregion
