...
guards), `theme`, `feedback` (the app-wide message queue), `request-state` (CallState
...
features/<feature>/
  ...
  ui/
    pages/                # optional
    components/           # optional
    tables/               # optional — hlmTable grids for the feature's entity collections
    dataviews/            # optional — list/grid browsing surfaces
    forms/                # optional
    dialogs/              # optional
    sheets/              # optional
  state/
...
- create `ui/tables/` for `hlmTable`-based tabular grids of the feature's entity collections (columns, rows, row menus, sorting, pagination) and `ui/dataviews/` for list/grid card browsing surfaces; both are presentational (section 10.3),
...
features/<parent>/features/<child>/
  ...
  ui/
    pages/                # optional
    components/           # optional
    tables/               # optional — hlmTable grids
    dataviews/            # optional — list/grid browsing surfaces
    forms/                # optional
    dialogs/              # optional
    sheets/              # optional
    ...
    ui/                      # only when the concept renders something
      ...
      directives/
        <name>/
          ...
          <name>.directive.ts
          testing/
      pipes/                 # e.g. shared/regional-format/ui/pipes/org-date/
...
A concept with **no UI** creates no `ui/` and stays flat (a validator, a util, a type, or a constants file plus their barrel — like `core/boot-readiness`). Illustrative concepts in this codebase: `layout-slot` (the slot mechanism the layouts are built on — section 8.2), `empty-state`, `error-state`, `identity-summary`, `inplace-field` (the edit-in-place shell of section 10.5), `password-input`, `theme-switcher`, `regional-format` (the `OrgDatePipe` concept — `ui/pipes/` only, plus the flat `RegionalFormatSettings` type and its default; a domain-agnostic formatting context supplied by a feature-published port, never imported directly — section 9.2, section 14.3).
...
**A shared wrapper must earn its place.** A `shared` component that exists only so call sites avoid repeating markup does not: put the markup at each call site and accept the duplication. Wrapping a spartan component to rename it is the clearest case of a wrapper that earns nothing. A wrapper is justified only by a capability nothing else expresses (a per-drop validation predicate, a scheduler, a stacking deck), a rendering shape no existing component covers, or an accessibility pattern that must not be got wrong twice (a navigation list that must not be a `role="menu"`).

`empty-state` and `error-state` are the worked example of the third case, and of what a legitimate wrapper looks like once the design system catches up. Both are now **compositions of spartan's `empty`** — `hlmEmpty` › `hlmEmptyHeader` (`hlmEmptyMedia` + `hlmEmptyTitle` + `hlmEmptyDescription`) › `hlmEmptyContent` — not markup of their own. What they still earn is the contract around it: three inputs instead of a six-element composition repeated at thirteen call sites, and, for `error-state`, `role="alert"` plus the destructive tint. Getting a failure to announce itself is the part that must not be got wrong thirteen times; the shape is spartan's.
...
External consumers import a shared concept through its barrel — `@shared/empty-state`, `@shared/inplace-field`, `@shared/theme-switcher` — exactly as they import `@core/theme` or `@core/api`. There is no root `@shared` barrel and no aggregate kind barrel. Cross-concept imports inside `shared` also go through the sibling concept's barrel, mirroring `core → core`.
...
| `.directive.ts`   | attribute directives, inside their own folder under `shared/<concept>/ui/directives/<name>/` (or the owning feature's `ui/` when domain-aware) | `gate-reason.directive.ts` (`shared/gate-reason/ui/directives/gate-reason/`) |
...
| `.pipe.ts`        | pure Angular pipes, inside their own folder under `shared/<concept>/ui/pipes/<name>/`                                                          | `org-date.pipe.ts` (`shared/regional-format/ui/pipes/org-date/`)             |
...
- pipes keep the `Pipe` suffix, patterning with directives rather than dropping it like components: `OrgDatePipe`. The `@Pipe({ name: … })` value is `camelCase` and prefixed `app` (`appOrgDate`) to stay unambiguous against Angular's built-ins (`date`, `slice`, `async`) — section 9.4 governs selectors and does not itself reach a pipe name, so this prefix is the same judgment call as the suffix, recorded here per section 14.3. A pipe is always pure (`pure: false` is never set) and takes no dependencies — a pipe needing a service takes its value as an explicit argument instead, so a caller supplies it from a `computed()` over that service,
...
`ui/tables/` and `ui/dataviews/` both render the feature's entity collections as presentational components: they receive collection state through `input()` signals and emit paging, sorting, filtering, selection, and action events through `output()`, and neither injects feature stores nor calls data-access services. They differ only by the rendering shape:

- `ui/tables/` — **tabular grids** composed from the `hlmTable` primitives of `@shared/ui/table`. Use for data-dense administrative listings of one entity.
- `ui/dataviews/` — **list/grid browsing surfaces** with a layout toggle and card-style items. Use for scannable, presentation-oriented collection browsing.

The `hlmTable` primitives are **styling only**: they carry no sorting, grouping, collapsing, selection or pagination. A `ui/tables/` surface owns those itself, as semantic markup exposed through `output()` — a sort button inside the `<th>` with `aria-sort` on the header, a group header row whose button carries `aria-expanded`/`aria-controls`, one `<tbody>` per group. **Row navigation is a stretched `routerLink` anchor** (`after:absolute after:inset-0` on a `relative` row), never a `role="button"` row: one tab stop, a real href, and middle-click works.
...
- an in-place field owns the display/edit toggle and its close affordance;
  that shell is `@shared/inplace-field`, which projects the resting rendering and
  the control and owns Escape-reverts-and-returns-focus, the pending lock and
  the error line — it never closes itself after a save, because only the host
...
- `shared/<concept>/index.ts` for each shared concept (`@shared/empty-state`, `@shared/inplace-field`, `@shared/error-state`, …) — `shared` has no root barrel and no aggregate kind barrel (section 8.5).
