...
export const AssistantStore = signalStore(
  ...
  withMethods(
    (
      ...
      /** Bare id of the organization the panel is scoped to. */
      function organizationId(): string | null {
        return organizationContext.selectedOrganization()?.id ?? null;
      }
      ...
      return {
         ...
         *
         * Driven by the organization signal, so switching organization drops
         * the previous transcript instead of showing it under the wrong tenant.
         */
        resume: rxMethod<OrganizationOutput | null>(
          tap((organization: OrganizationOutput | null): void => {
            patchState(store, { ...INITIAL_THREAD_STATE });

            if (organization === null) return;

            const remembered: string | null = cookies.getCookie<string>(
              cookieName(organization.id),
            );
  ...
  withHooks(
    (store, organizationContext = inject<OrganizationContextPort>(ORGANIZATION_CONTEXT_PORT)) => ({
      onInit(): void {
        store.resume(organizationContext.selectedOrganization);
      },
    }),
  ),
);
